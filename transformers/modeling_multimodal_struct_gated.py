...
class BertLayer(nn.Module):
    def __init__(self, config, hgn_config, layer_no):
        ...
        ######################   SHOW CONFIG FILES
        # logger.info("SHOW Transformer Config")
        # logger.info(config)
        # logger.info("SHOW HGN Config")
        # logger.info(hgn_config)
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(txt_hidden_states, attention_mask, head_mask)
        ...
        attention_out_text  =  attention_output

        
        self_graph_attention_outputs = self.attention(graph_hidden_states, attention_mask, head_mask)
        attention_out_graph = self_graph_attention_outputs[0]
        outputs = self_graph_attention_outputs[1:]  # add self attentions if we output attention weights
            # ------------------>  WHY this part?
            # if self.is_decoder and encoder_hidden_states is not None:
            #     cross_attention_outputs = self.crossattention(
            #         attention_output, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask
        ...
        ######## Bottom Adapter
        attention_output_text = self.adapter_text_bottom(attention_out_text)
        attention_output_graph = self.adapter_graph_bottom(attention_out_graph)
        ######     NEW END
        ...
        layer_output_text = self.dropout(self.projection(layer_output)) # Inverted Bottle-neck layer
        ...
        outputs_graph = (layer_output_graph,) + outputs_graph
