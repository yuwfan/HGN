...
from transformers import get_linear_schedule_with_warmup, MultiModal_Roberta_v4_unbalanced, AdamW
...
run = neptune.init(tags=["MultiModal_base", "Fusion_Module"])
...
def calc_params(params):
    ...
    Init_Model(ar)
    model = MultiModal_Roberta_v4_unbalanced(ar)
    result = get_t_p(model, print_stats=True)
    del model
    return result


# for i in range(99,160):
#     with open("Parameter/MMv4_d1.txt", 'a') as f:
...
model = MultiModal_Roberta_v4_unbalanced(args)
model.to(args.device)

# #######################     Check sizes of model:   #############################
# # Step by step instructions for new Model with different size:
# # 1.) Name it and chenge args
# a2=change_argument([('--adapter_size','135'),('--hgn_hidden_size', '540')],None)
# # 2.) Initialize model
# Init_Model(a2)
# # 3.) Create model
# model = MultiModal_Roberta_v4_unbalanced(a2)
# # 4.) Output parameters
# optimizer = get_optimizer(model, a2, learning_rate, remove_pooler=False)
# params_name, params = get_training_params(model, print_stats=True)
# ################################################################################
...
##############################
# Methods for checking gradients:

gradients_filename = "gradients/multi_v4.txt"
def check_gradients(model_temp):

    for n, p in model.named_parameters():
        params = parameters_to_vector(p)
        mean = torch.mean(params)
        std = torch.std(params)

        with open(gradients_filename, 'a') as a:
            a.write(f"{n} {mean.item()} {std.item()}\n")
    with open(gradients_filename, 'a') as a:
        a.write("\n")
