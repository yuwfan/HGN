...
DATA_ROOT=/ssd2/data/MultiHopQA
# define precached BERT MODEL path, if not downloaded just just 'bert-base-cased'
BERT_BASE=$DATA_ROOT/models/pretrained/bert-base-cased/
BERT_BASE_UNCASED=$DATA_ROOT/models/pretrained/bert-base-uncased/
ROBERTA_LARGE=$DATA_ROOT/models/pretrained/roberta-large
# paragraph ranker
PARA_EVAL_CKPT=$DATA_ROOT/models/finetuned/para_select_model.bin
...
for input in ${INPUTS[*]}; do
    ...
    python scripts/3_paragraph_ranking.py --data_dir $OUTPUT_PROCESSED --eval_ckpt $DATA_ROOT/models/finetuned/PS/pytorch_model.bin --raw_data $INPUT_FILE --input_data $OUTPUT_PROCESSED/hotpot_ss_$DATA_TYPE.csv --model_name_or_path roberta-large --model_type roberta --per_gpu_eval_batch_size 64 --max_seq_length 256 --fp16
    ...
    python scripts/5_dump_features.py --para_path=$OUTPUT_PROCESSED/multihop_para.json --full_data=$INPUT_FILE --model_name_or_path roberta-large --do_lower_case --ner_path $OUTPUT_PROCESSED/ner.json --model_type roberta --tokenizer_name roberta-large --output_dir $OUTPUT_FEAT --doc_link_ner $OUTPUT_PROCESSED/doc_link_ner.json --graph_id 0

    echo "6. Test dumped features"
    python scripts/6_test_features.py --raw_data $INPUT_FILE --input_dir $OUTPUT_FEAT --output_dir $OUTPUT_FEAT --model_type roberta --graph_id 0 --model_name_or_path roberta-large
done
