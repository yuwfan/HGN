...
from transformers import get_linear_schedule_with_warmup, MultiModalStructAdaptFastRoberta_v2_woBoAd_wt, AdamW
...
def get_training_params(graphqa, print_stats=False):
    ...
    if print_stats:
        ...
        logger.info(f"-----------------------")
        logger.info(f"Ratio txt/graph: {dict_params['text']/dict_params['graph']:.2f}")
        logger.info(f"Ratio txt/sum: {(dict_params['text']/(dict_params['text']+dict_params['graph'])):.2f}")
...
model = MultiModalStructAdaptFastRoberta_v2_woBoAd_wt(args)
...
def calc_params(params):
    ...
    Init_Model(ar)
    model = MultiModalStructAdaptFastRoberta_v2_woBoAd_wt(ar)
...
def new_model(adapt = '86', hid='300'):
    ...
    Init_Model(a13)
    model = MultiModalStructAdaptFastRoberta_v2_woBoAd_wt(a13)
...
# Init_Model(a34)
# # 3.) Create model
# model = MultiModalStructAdaptFastRoberta_v2_woBoAd_wt(a34)
# # 4.) Output parameters
