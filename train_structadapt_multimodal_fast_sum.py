...
from transformers import get_linear_schedule_with_warmup, MultiModalStructAdaptFastRoberta_v2_sum, AdamW
...
def all_params(graphqa, print_stats=False):
    params_name_frozen = []
    ...
    #     params.append(p)
    #     params_name.append(n)
    logger.info(f"Summary of parameters distribution")
    for k, v in dict_params.items():
        logger.info(f"Number of {k} parameters: {v/1e6:.2f}M")
    logger.info(f"Total number of trained params are:  {num_params/1e6:.2f}M")
...
model = MultiModalStructAdaptFastRoberta_v2_sum(args)
...
def calc_params(params):
    ...
    Init_Model(ar)
    model = MultiModalStructAdaptFastRoberta_v2_sum(ar)
...
def new_model(adapt = '86', hid='300'):
    ...
    Init_Model(a13)
    model = MultiModalStructAdaptFastRoberta_v2_sum(a13)
    optimizer = get_optimizer(model, a13, learning_rate, remove_pooler=False)

# new_model(adapt = '128', hid='85')
...
# Init_Model(a34)
# # 3.) Create model
# model = MultiModalStructAdaptFastRoberta_v2_sum(a34)
# # 4.) Output parameters
